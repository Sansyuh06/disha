...
Generate 3-5 steps appropriate for the task. Use realistic Union Bank counter names. Be specific. Return only the JSON object.`,
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto">
      {/* Header */}
      <div className="mb-8">
        <h1 className="font-heading text-3xl font-semibold mb-1" style={{ color: 'var(--navy-900)' }}>Plan Your Branch Visit</h1>
        <p className="text-sm" style={{ color: 'var(--text-muted)' }}>Tell us what you need and we'll map your route</p>
      </div>

      {/* Input Card */}
      <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm mb-6">
        <textarea
          className="w-full border border-gray-200 rounded-xl p-4 text-sm resize-none focus:outline-none transition-colors"
        ...
        <div className="flex items-center gap-3 mt-3">
          ...
          <button
            ...
            style={{ borderColor: micState === 'listening' ? '#16A34A' : 'var(--teal)' }}
          ...
          >
            ...
            {micState === 'processing' ? (
              ...
              <svg className="w-5 h-5" style={{ color: micState === 'listening' ? '#16A34A' : 'var(--teal)' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <PresetChips onSelect={(text: string) => { setInput(text); handleSubmit(text); }} />
      </div>
      ...
      {result && !loading && (
        <AnimatePresence>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            {/* Summary card */}
            <div className="bg-white rounded-2xl border-l-4 p-5 mb-5 shadow-sm" style={{ borderLeftColor: 'var(--teal)' }}>
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-heading font-semibold" style={{ color: 'var(--navy-900)' }}>{result.task_summary}</p>
                  <p className="text-sm mt-0.5" style={{ color: 'var(--text-muted)' }}>Estimated total: ~{result.total_minutes} minutes</p>
                </div>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="w-24 h-2 bg-gray-100 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{ width: `${completion}%`, backgroundColor: 'var(--teal)' }}
                      />
                    </div>
                    <span className="text-xs font-medium" style={{ color: 'var(--navy-900)' }}>{completion}%</span>
                  </div>
                </div>
              </div>
            </div>
