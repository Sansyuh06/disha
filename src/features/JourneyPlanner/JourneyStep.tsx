    ...
    <div className="relative pl-14">
      ...
      <div
        ...
        style={{
          ...
          borderColor: completed ? '#16A34A' : 'var(--teal)',
          color: completed ? 'white' : 'var(--teal)',
        }}
      >
        {completed ? '✓' : step.step}
      </div>
      ...
      >
        <div className="flex items-start justify-between gap-3 mb-2">
          <div className="flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              <h4 className={`font-heading font-semibold text-[var(--navy-900)] ${completed ? 'line-through text-[var(--text-muted)]' : ''}`}>
                {step.counter}
              </h4>
            ...
            <p className="text-sm text-[var(--text-muted)] mt-0.5">{step.service}</p>
          </div>
          {!completed && (
            <button
              onClick={onMarkDone}
        ...
        <p className="text-sm text-[var(--navy-900)] mb-3">{step.purpose}</p>
        ...
        {step.documents.length > 0 && (
          <div className="border border-gray-100 rounded-xl overflow-hidden mb-3">
            ...
            >
              ...
              <span className="text-[var(--text-muted)]">{docsOpen ? '▲' : '▼'}</span>
            </button>
            {docsOpen && (
              <div className="px-4 py-3 space-y-2">
                {step.documents.map((doc, i) => (
                  <label key={i} className="flex items-center gap-2 text-sm cursor-pointer">
                    ...
                    <span className="text-[var(--navy-900)]">{doc}</span>
                  </label>
                ))}
              </div>
            )}
        ...
        {/* Tip */}
        {step.tip && (
          <p className="text-xs italic flex items-start gap-2" style={{ color: 'var(--teal)' }}>
            <span>💡</span>
            <span>{step.tip}</span>
          </p>
        )}
