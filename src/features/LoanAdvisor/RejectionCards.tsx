...
export default function RejectionCards({ reasons }: { reasons: Reason[] }) {
  return (
    <div className="space-y-4">
      <h3 className="font-heading font-semibold text-[var(--navy-900)]">Why your application needs improvement</h3>
      {reasons.map((r, i) => {
        ...
        return (
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <h4 className="font-heading font-semibold text-[var(--navy-900)]">{r.title}</h4>
              <span
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div className="bg-white rounded-xl p-3 border border-red-100">
                <p className="text-xs text-[var(--text-muted)] mb-0.5">Your value</p>
                <p className="font-bold text-red-600 text-lg font-heading">{r.your_value}</p>
              </div>
              <div className="bg-white rounded-xl p-3 border border-green-100">
                <p className="text-xs text-[var(--text-muted)] mb-0.5">Required</p>
                <p className="font-bold text-green-600 text-lg font-heading">{r.required}</p>
              </div>
            </div>

            <p className="text-sm text-[var(--navy-900)] mb-3 leading-relaxed">{r.plain_explanation}</p>
