...
Create a realistic financial plan. Return ONLY this JSON:
{
  ...
  return (
    <div className="p-6 max-w-2xl mx-auto">
      <div className="mb-8">
        <h1 className="font-heading text-3xl font-semibold text-[var(--navy-900)] mb-1">First Salary Coach</h1>
        <p className="text-[var(--text-muted)] text-sm">Build smart money habits from day one</p>
      </div>
          ...
          <React.Fragment key={i}>
            <div className="flex items-center gap-2">
              <div
                ...
                style={{
                  backgroundColor: i <= currentStep ? 'var(--teal)' : '#E2E8F0',
                  color: i <= currentStep ? 'white' : 'var(--text-muted)',
                }}
              >
                {i < currentStep ? '✓' : i + 1}
              </div>
              <span className="text-sm font-medium hidden sm:block" style={{ color: i === currentStep ? 'var(--teal)' : 'var(--text-muted)' }}>
                {label}
              </span>
            </div>
        ...
        {currentStep === 2 && plan && (
          <motion.div key="step2" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
              ...
              {plan.goals.map((g, i) => (
                <div key={i} className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="font-heading font-semibold text-[var(--navy-900)] text-sm">{g.name}</h4>
                    <span className="text-sm font-bold" style={{ color: 'var(--teal)' }}>₹{g.monthly_contribution.toLocaleString('en-IN')}/mo</span>
                  </div>
                  <p className="text-xs text-[var(--text-muted)]">Reach your goal in {g.months_to_achieve} months · {g.account_type}</p>
                  <div className="h-1.5 bg-gray-100 rounded-full mt-2">
                    <div className="h-full rounded-full w-0" style={{ backgroundColor: 'var(--teal)' }} />
                  </div>
                </div>
              ))}
            </div>
              ...
              <div className="space-y-2">
                {plan.first_month_actions.map((a, i) => (
                  <label key={i} className="flex items-start gap-2 cursor-pointer">
                    ...
                    <span className="text-sm text-[var(--navy-900)]">{a}</span>
                  </label>
                ))}
              </div>
            </div>
            ...
            <details className="bg-white rounded-2xl border border-gray-100 shadow-sm mb-5">
              <summary className="px-5 py-4 cursor-pointer font-semibold font-heading text-sm text-[var(--navy-900)]">
                🏦 Recommended Union Bank Products
              </summary>
              <div className="px-5 pb-4 space-y-3">
                {plan.union_bank_products.map((p, i) => (
                  <div key={i} className="border-l-2 pl-3" style={{ borderLeftColor: 'var(--teal)' }}>
                    <p className="text-sm font-semibold text-[var(--navy-900)]">{p.product}</p>
                    <p className="text-xs text-[var(--text-muted)]">{p.why_relevant}</p>
                    <span className="text-[10px] px-2 py-0.5 rounded-full mt-1 inline-block" style={{ backgroundColor: 'var(--teal-light)', color: 'var(--teal)' }}>{p.counter}</span>
                  </div>
                ))}
              </div>
            </details>
            ...
            {plan.motivational_message && (
              <blockquote className="border-l-4 pl-4 italic text-sm mb-5" style={{ borderLeftColor: 'var(--teal)', color: 'var(--teal)' }}>
                "{plan.motivational_message}"
              </blockquote>
            )}
